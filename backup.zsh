...
printf '%s' $1 | hdiutil create -encryption AES-128 -stdinpass -size ${VOLSIZE:-'10g'} -fs APFS -volname transfer $HOME/transfer.dmg
...
cp -r $HOME/{.aws,.kube,.ssh,.vault*,.zprofile,.zshrc,Documents,Desktop,Downloads,development} /Volumes/transfer
